...
function encode2(ver, ecc, data, type, encoder = utf8ToBytes) {
  let encoded = "";
...
var profile = await messagesStore.get("profile") ?? { name: "" };
...
document.getElementById("acceptOffer").addEventListener("click", async () => {
  ...
  await peer.ready();
  peer.send({ type: "snapshot", payload: messages.toJSON() });
});
...
messages.addEventListener("delta", ({ detail }) => {
  if (peer) peer.send({ type: "delta", payload: detail });
  messages.snapshot();
});
